...
const modalCopy = {
  en: {
    ...
    email: 'Email', loginByEmail: 'Email', loginByPhone: 'Phone', phoneNumber: 'Phone number', countryCode: 'Country / region code', password: 'Password', confirmPassword: 'Confirm password',
    google: 'Sign in with Google', apple: 'Sign in with Apple', code: 'Verification code', sendCode: 'Send code',
  ...
  zh: {
    ...
    loginTitle: '欢迎回来！', loginSubtitle: '使用邮箱或手机号登录', loginSubmit: '进入 NovaOS',
    ...
    email: '邮箱', loginByEmail: '邮箱登录', loginByPhone: '手机号登录', phoneNumber: '手机号', countryCode: '国家 / 地区码', password: '密码', confirmPassword: '确认密码',
    google: '使用 Google 登录', apple: '使用 Apple 登录', code: '验证码', sendCode: '发送验证码',
  ...
  ja: {
    ...
    loginTitle: 'おかえりなさい', loginSubtitle: 'メールまたは電話番号でログイン', loginSubmit: 'NovaOS に入る',
    ...
    email: 'メール', loginByEmail: 'メール', loginByPhone: '電話番号', phoneNumber: '電話番号', countryCode: '国 / 地域コード', password: 'パスワード', confirmPassword: 'パスワード確認',
    google: 'Google でログイン', apple: 'Apple でログイン', code: '認証コード', sendCode: 'コードを送信',
    noAccount: 'アカウントがありませんか？', haveAccount: 'すでにアカウントをお持ちですか？', register: 'アクセス作成', login: 'ログイン', remember: '30日間記憶', forgot: 'パスワードをお忘れですか？',
    ...
    loginModules: ['GEO可視性', 'コンテンツキュー', 'リード受信箱', '成長レポート'],
    registerModules: ['ブランド情報', '初回GEO診断', 'コンテンツ計画', 'リード獲得'],
    visualTags: ['ブランド', 'GEO', 'リード'],
  },
  ko: {
    eyebrow: 'NovaOS access', close: '닫기',
    loginTitle: '다시 오신 것을 환영합니다', loginSubtitle: '이메일 또는 전화번호로 로그인하세요', loginSubmit: 'NovaOS 보기',
    ...
    email: '이메일', loginByEmail: '이메일', loginByPhone: '전화번호', phoneNumber: '전화번호', countryCode: '국가 / 지역 코드', password: '비밀번호', confirmPassword: '비밀번호 확인',
    google: 'Google로 로그인', apple: 'Apple로 로그인', code: '인증 코드', sendCode: '코드 보내기',
    noAccount: '계정이 없나요?', haveAccount: '이미 계정이 있나요?', register: '접근 만들기', login: '로그인', remember: '30일 기억', forgot: '비밀번호를 잊으셨나요?',
    ...
    loginModules: ['GEO 가시성', '콘텐츠 큐', '리드 받은함', '성장 리포트'],
    registerModules: ['브랜드 메모리', '첫 GEO 진단', '콘텐츠 계획', '리드 확보'],
    visualTags: ['브랜드', 'GEO', '리드'],
  },
} as const;
...
function countryName(item: (typeof countryCodes)[number], lang: Lang) {
...
function EyeIcon({ hidden }: { hidden: boolean }) {
  return hidden ? (
    ...
    <svg viewBox="0 0 24 24" className="h-5 w-5" aria-hidden="true"><path d="M2 12s3.6-7 10-7 10 7 10 7-3.6 7-10 7S2 12 2 12Z" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /><circle cx="12" cy="12" r="3" fill="none" stroke="currentColor" strokeWidth="1.8" /></svg>
  );
}
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-[1fr_1.05fr]">
      <label className="grid gap-2 text-sm font-semibold text-slate-700">
        {copy.countryCode}
        <select value={countryCode} onChange={(event) => setCountryCode(event.target.value)} className="border-0 border-b border-slate-200 bg-white px-0 py-3 text-slate-700 outline-none transition focus:border-blue-500">
          {countryCodes.map((item) => <option key={`${item.country}-${item.code}`} value={item.code}>{countryName(item, lang)} {item.code}</option>)}
        </select>
      </label>
      <label className="grid gap-2 text-sm font-semibold text-slate-700">
        {copy.phoneNumber}
        <input value={phone} onChange={(event) => setPhone(event.target.value.replace(/[^0-9\s-]/g, ''))} type="tel" inputMode="tel" autoComplete="tel" className="border-0 border-b border-slate-200 px-0 py-3 text-slate-700 outline-none transition focus:border-blue-500" />
      </label>
    </div>
  );
}
  ...
  return (
    <div className="relative hidden min-h-[650px] overflow-hidden bg-[radial-gradient(circle_at_18%_14%,rgba(37,99,235,0.22),transparent_20rem),radial-gradient(circle_at_82%_78%,rgba(124,58,237,0.16),transparent_24rem),linear-gradient(135deg,#f5f8ff_0%,#eaf0ff_56%,#fbfdff_100%)] p-10 lg:block">
      ...
      <div className="relative flex h-full flex-col justify-between">
        ...
        <div className="flex gap-8 text-sm font-medium text-slate-500"><a href="/privacy">{copy.footerPrivacy}</a><a href="/privacy#terms">{copy.footerTerms}</a><a href="/contact">{copy.footerContact}</a></div>
      </div>
    </div>
  );
}
...
export function MaintenanceModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 px-4 backdrop-blur-md" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <div ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="novaos-login-title" className="relative grid w-full max-w-6xl overflow-hidden rounded-[2rem] border border-white/20 bg-white shadow-2xl shadow-slate-950/30 outline-none lg:grid-cols-[1.05fr_1fr]">
        ...
        <form onSubmit={handleSubmit} className="flex min-h-[650px] items-center justify-center p-8 lg:p-12">
          <div className="w-full max-w-[28rem]">
            <div className="mb-8 text-center"><div className="mx-auto mb-7 flex h-10 w-10 items-center justify-center rounded-full text-2xl text-slate-900">✦</div><p className="mb-3 text-xs font-semibold uppercase tracking-[0.24em] text-blue-600">{copy.eyebrow}</p><h2 id="novaos-login-title" className="text-3xl font-bold tracking-tight text-slate-950">{title}</h2><p className="mt-3 text-sm text-slate-500">{subtitle}</p></div>
            <div className="grid gap-4">
              {mode === 'login' ? (
                <>
                  <div className="grid grid-cols-2 rounded-2xl bg-slate-100 p-1 text-sm font-semibold">
                    <button type="button" onClick={() => setLoginMethod('email')} className={`rounded-xl px-4 py-2.5 transition ${loginMethod === 'email' ? 'bg-white text-slate-950 shadow-sm' : 'text-slate-500 hover:text-slate-950'}`}>{copy.loginByEmail}</button>
                    <button type="button" onClick={() => setLoginMethod('phone')} className={`rounded-xl px-4 py-2.5 transition ${loginMethod === 'phone' ? 'bg-white text-slate-950 shadow-sm' : 'text-slate-500 hover:text-slate-950'}`}>{copy.loginByPhone}</button>
                  </div>
                  {loginMethod === 'email' ? (
                    <label className="grid gap-2 text-sm font-semibold text-slate-700">{copy.email}<input value={email} onChange={(event) => setEmail(event.target.value)} type="email" autoComplete="email" className="border-0 border-b border-slate-200 px-0 py-3 text-slate-700 outline-none transition focus:border-blue-500" /></label>
                  ) : (
                    <PhoneFields copy={copy} lang={lang} countryCode={loginCountryCode} setCountryCode={setLoginCountryCode} phone={phone} setPhone={setPhone} />
                  )}
                  <label className="grid gap-2 text-sm font-semibold text-slate-700">{copy.password}<div className="flex border-b border-slate-200 focus-within:border-blue-500"><input value={password} onChange={(event) => setPassword(event.target.value)} type={showPassword ? 'text' : 'password'} autoComplete="current-password" className="min-w-0 flex-1 border-0 px-0 py-3 text-slate-700 outline-none" /><button type="button" onClick={() => setShowPassword((value) => !value)} className="flex items-center px-2 text-slate-400 transition hover:text-blue-700" aria-label={showPassword ? copy.hidePassword : copy.showPassword}><EyeIcon hidden={showPassword} /></button></div></label>
                </>
              ) : (
                <>
                  <label className="grid gap-2 text-sm font-semibold text-slate-700">{copy.email}<input value={email} onChange={(event) => setEmail(event.target.value)} type="email" autoComplete="email" className="border-0 border-b border-slate-200 px-0 py-3 text-slate-700 outline-none transition focus:border-blue-500" /></label>
                  <PhoneFields copy={copy} lang={lang} countryCode={registerCountryCode} setCountryCode={setRegisterCountryCode} phone={registerPhone} setPhone={setRegisterPhone} />
                  <div className="grid gap-3 sm:grid-cols-[1fr_auto]"><label className="grid gap-2 text-sm font-semibold text-slate-700">{copy.code}<input value={code} maxLength={6} onChange={(event) => setCode(event.target.value.replace(/\D/g, '').slice(0, 6))} inputMode="numeric" autoComplete="one-time-code" className="border-0 border-b border-slate-200 px-0 py-3 text-slate-700 outline-none transition focus:border-blue-500" /></label><button type="button" className="self-end rounded-xl border border-slate-200 bg-slate-50 px-5 py-3 text-sm font-semibold text-blue-700 transition hover:bg-blue-50">{copy.sendCode}</button></div>
                  <label className="grid gap-2 text-sm font-semibold text-slate-700">{copy.password}<input value={password} onChange={(event) => setPassword(event.target.value)} type="password" autoComplete="new-password" className="border-0 border-b border-slate-200 px-0 py-3 text-slate-700 outline-none transition focus:border-blue-500" /></label>
            ...
            <div className="mt-5 flex items-center justify-between text-sm"><label className="flex items-center gap-2 text-slate-500"><input type="checkbox" defaultChecked className="h-4 w-4 accent-blue-600" />{copy.remember}</label>{mode === 'login' ? <a href="#" className="font-semibold text-blue-700">{copy.forgot}</a> : null}</div>
            <label className="mt-4 flex items-start gap-3 text-xs leading-5 text-slate-500"><input type="checkbox" checked={agreed} onChange={(event) => setAgreed(event.target.checked)} required className="mt-0.5 h-4 w-4 shrink-0 accent-blue-600" /><span>{copy.agreePrefix} <a href="/privacy" className="font-semibold text-blue-700" target="_blank" rel="noreferrer">{copy.privacyPolicy}</a> {copy.agreeAnd} <a href="/privacy#terms" className="font-semibold text-blue-700" target="_blank" rel="noreferrer">{copy.serviceTerms}</a></span></label>
            ...
            <p className="mt-8 text-center text-sm text-slate-500">{mode === 'login' ? copy.noAccount : copy.haveAccount}{' '}<button type="button" onClick={() => resetMode(mode === 'login' ? 'register' : 'login')} className="font-semibold text-slate-950">{mode === 'login' ? copy.register : copy.login}</button></p>
          </div>
        </form>
      </div>
    </div>
