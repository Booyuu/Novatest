...
export function AIAssistantBar() {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    // TODO: Connect real AI assistant later.
    // Example:
    // await fetch('/api/nova-assistant', { method: 'POST', body: JSON.stringify({ query }) });
    window.alert('AI assistant API placeholder: ' + (query || t.ai.input));
  }

  return (
    <section className="bg-gradient-to-r from-blue-600 via-indigo-500 to-violet-500 px-5 py-7 text-white sm:px-8 lg:px-10">
      <div className="mx-auto grid max-w-[1500px] items-center gap-6 lg:grid-cols-[0.26fr_0.74fr]">
        <div className="flex items-center gap-4">
          <div className="flex h-20 w-20 items-center justify-center rounded-full bg-white/20 text-4xl shadow-xl backdrop-blur-xl">🤖</div>
          <div>
